...
def update_cloudsql_from_bigquery(config, postproc_config, project_name, cloudsql_table, log, data_type = None, endpt_type = None):
    ...
    if project_name:
        if data_type: # assumes that endpt_type is also supplied
            query_results = query_bq_table(postproc_config['postproc_query'] % (', '.join(postproc_config['postproc_columns'].keys()), endpt_type, config['cloud_projects']['open'], data_type), False, postproc_config['postproc_project'], log)
        else:
